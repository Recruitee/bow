defmodule Bow do
  ...
  # -- before moving to umbrella config needs to be nested inside :recruitee key
  def storage,          do: Keyword.get(Application.get_env(:recruitee, :bow, []), :storage, Bow.Storage.Local)
  def store_timeout,    do: Keyword.get(Application.get_env(:recruitee, :bow, []), :store_timeout, 30_000)
  def version_timeout,  do: Keyword.get(Application.get_env(:recruitee, :bow, []), :version_timeout, 60_000)

  # uncomment the following lines after extracting bow into separate application
  ...
  def load(file) do
    %{file | path: get_file_path(file.uploader, file)}
  end
